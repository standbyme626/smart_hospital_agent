...
try:
    ...
    print(
        "[Warning] Failed to import optimum.onnxruntime. ONNX support will be disabled."
    )
...
class QwenReranker:
    """
    Qwen3-Reranker 封装类 (支持 ONNX 加速 + SiliconFlow 云端)
    针对 Qwen3-Reranker 的 CausalLM 架构进行了优化，使用 yes/no logit 评分

    [Refactor Phase 2] Added Async Support
    [Cloud] Added SiliconFlow remote reranking support
    """
    ...
    def __init__(self, model_path: str):
        ...
        self.siliconflow_rerank_model = str(
            getattr(settings, "SILICONFLOW_RERANK_MODEL", "Qwen/Qwen3-Reranker-0.6B")
        )
    ...
    def _reload(self):
        ...
        try:
            ...
            self.token_yes_id = self.tokenizer.encode("Yes", add_special_tokens=False)[
                0
            ]
                ...
                try:
                    ...
                    if ORTModelForCausalLM:
                        ...
                        self.onnx_input_names = {
                            i.name for i in self.model.get_inputs()
                        }
            ...
            print(
                f"[Reranker] Failed to load model: {e}. Switching to Passthrough Mode."
            )
            self.model = None
    ...
    def rerank(self, query: str, docs: list[dict], top_k: int = 3) -> list[dict]:
        """
        使用 CausalLM 架构进行重排序评分 (支持本地/SiliconFlow云端)
        """
        if not docs:
            return []

        # Use SiliconFlow Cloud API if configured
        ...
        formatted_texts = [
            self._format_input(query, d.get("content", "")) for d in docs
        ]
        ...
        if self.use_onnx and self.use_onnx_runtime:
            ...
            if (
                "position_ids" in self.onnx_input_names
    ...
    def _rerank_siliconflow(
        ...
        """SiliconFlow Cloud API reranking"""
        import requests
        ...
        # SiliconFlow rerank API format
        payload = {
            "model": self.siliconflow_rerank_model,
            "query": query,
        ...
        try:
            response = requests.post(
                f"{self.siliconflow_api_base}/rerank",
                json=payload,
                headers=headers,
                timeout=30,
            )
            ...
            print(f"[Reranker] SiliconFlow rerank failed: {e}")
            # Fallback: return original docs with default scores
            for d in docs:
                d["score"] = d.get("score", 0.5)
                d["source"] = d.get("source", "vector")
            docs.sort(key=lambda x: x.get("score", 0.0), reverse=True)
            return docs[:top_k]
