...
from app.core.middleware.instrumentation import (
...
class MedicalRetriever:
    ...
    def __init__(self):
        ...
        # 检查 Reranker 模型是否存在 或 配置了 SiliconFlow
        rerank_path = settings.RERANKER_MODEL_PATH
        siliconflow_key = getattr(settings, "SILICONFLOW_API_KEY", "")

        if siliconflow_key or os.path.exists(rerank_path):
    ...
    def _default_dept_post_debug(self) -> Dict[str, Any]:
        return {
            "dept_normalize_enabled": bool(
                getattr(settings, "RAG_DEPT_NORMALIZE_ON_RESULT", True)
            ),
            "dept_normalize_changed_count": 0,
            "dept_normalize_unknown_count": 0,
            "dept_normalize_reason": "not_checked",
            "dept_gate_enabled": bool(
                getattr(settings, "RAG_DEPT_CONSISTENCY_GATE_ON", True)
            ),
    ...
    def _build_department_keyword_index(self) -> Dict[str, set[str]]:
        ...
        for dept, keywords in extra_keywords.items():
            ...
            index.setdefault(canonical, set()).update(
                {k.strip().lower() for k in keywords if k}
            )

        return index

    def _department_keyword_support(
        self, query: str, canonical_dept: str
    ) -> tuple[int, List[str]]:
        keywords = self._department_keyword_index.get(canonical_dept, set())
        if not query or not keywords:
            return 0, []
        hits = sorted(
            {kw for kw in keywords if kw and kw in query}, key=len, reverse=True
        )
        ...
        for dept, keywords in self._department_keyword_index.items():
            ...
            if hit_count >= 2 or max_kw_len >= 3:
                keyword_candidates.append(
                    (-(hit_count + max_kw_len), -max_kw_len, dept)
                )
                seen.add(dept)
    ...
    def _default_pure_dept_penalty_debug(
        self, execution_path: str = "not_executed"
    ) -> Dict[str, Any]:
        return {
    ...
    def _extract_support_department_mentions(
        self, norm_query: str, top_k: int = 3
    ) -> List[str]:
        if not norm_query:
            return []
    ...
    def _select_pure_rerank_candidates(
        ...
        if not mentions:
            fallback_mentions = self._extract_support_department_mentions(
                norm_query, top_k=3
            )
        ...
        for dept in mentions:
            ...
            for idx, doc in enumerate(fused_results):
                if idx in reserved_indices:
                    continue
                canonical_dept, _ = self._normalize_department_name(
                    doc.get("department")
                )
    ...
    def _apply_pure_department_mismatch_penalty(
        ...
        debug["pure_dept_penalty_top1_before"] = str(
            results[0].get("department", "") or ""
        )
        ...
        debug["pure_dept_penalty_top1_after"] = str(
            results[0].get("department", "") or ""
        )
        return results, debug
    ...
    def _apply_department_consistency_gate(
        ...
        max_gap = max(
            0.0, float(getattr(settings, "RAG_DEPT_CONSISTENCY_MAX_SCORE_GAP", 0.12))
        )
        min_hits = max(
            1, int(getattr(settings, "RAG_DEPT_CONSISTENCY_MIN_KEYWORD_HITS", 1))
        )
        require_top1_unsupported = bool(
            getattr(settings, "RAG_DEPT_CONSISTENCY_REQUIRE_TOP1_UNSUPPORTED", True)
        )
        ...
        supported_candidates = [
            item for item in scored[1:] if int(item["support"]) >= min_hits
        ]
        if pure_mode and not supported_candidates:
            mentioned_candidates = [
                item for item in scored[1:] if bool(item.get("mentioned_in_query"))
            ]
        ...
        strong_supported_signal = (
            int(best_supported["support"]) >= max(min_hits + 1, 2)
            and support_delta >= 1
        )
        ...
        if (
            pure_mode
            and support_delta >= 1
            and int(best_supported["support"]) >= min_hits
        ):
            # pure 模式下，候选信号只要明显优于 top1，则允许更宽分差以减少误阻断。
            effective_max_gap = max(effective_max_gap, 0.85)
        if (
            pure_mode
            and int(top1["support"]) == 0
            and int(best_supported["support"]) >= 3
        ):
        ...
        if (
            pure_mode
            and int(top1["support"]) == 0
            and bool(best_supported.get("mentioned_in_query"))
        ):
    ...
    def _postprocess_department_results(
        ...
        results, gate_debug = self._apply_department_consistency_gate(
            query, results, pure_mode=pure_mode
        )
        debug.update(gate_debug)

        debug["dept_top1_after"] = (
            str(results[0].get("department", "") or "") if results else None
        )
        return results, debug
    ...
    def _build_cache_namespace(self) -> str:
        """构建缓存命名空间标签（版本化隔离）。"""
        raw = str(
            getattr(settings, "RAG_CACHE_NAMESPACE_VERSION", "v1") or "v1"
        ).strip()
    ...
    def _evaluate_cache_write_gate(
        ...
        require_known_dept = bool(
            getattr(settings, "RAG_CACHE_WRITE_REQUIRE_NON_UNKNOWN_DEPT", False)
        )
        debug = {
        ...
        second_score = (
            float(final_results[1].get("score", 0.0)) if len(final_results) > 1 else 0.0
        )
    ...
    async def _verify_semantic_cache_hit(
        ...
        """
        min_term_overlap = max(
            0, int(getattr(settings, "RAG_CACHE_VERIFY_MIN_TERM_OVERLAP", 1))
        )
        min_rerank_score = max(
            0.0, float(getattr(settings, "RAG_CACHE_VERIFY_MIN_RERANK_SCORE", 0.25))
        )
        max_doc_chars = max(
            100, int(getattr(settings, "RAG_CACHE_VERIFY_MAX_DOC_CHARS", 600))
        )
        ...
        top_content = (
            str(top_item.get("content", ""))
            if isinstance(top_item, dict)
            else str(cached_results[0])
        )
        ...
        if self.reranker:
            try:
                ...
                rerank_results = await asyncio.to_thread(
                    self.reranker.rerank, query, rerank_input
                )
                rerank_score = (
                    float(rerank_results[0].get("score", 0.0))
                    if rerank_results
                    else 0.0
                )
    ...
    async def analyze_intent(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if match:
                ...
                # 备选逻辑：关键词匹配
                res_upper = res.upper()
                if "GREETING" in res_upper:
                    return {"category": "GREETING"}
                if "OTHER" in res_upper:
            ...
            logger.error(
                "intent_analysis_failed", error=str(e), raw_res=locals().get("res", "")
            )
            return {"category": "MEDICAL"}
    ...
    async def search_rag30(
        ...
        try:
            ...
            pure_retrieval_mode = bool(
                getattr(settings, "RAG_PURE_RETRIEVAL_MODE", False)
            )
            if pure_mode is True:
                pure_retrieval_mode = True

            if pure_retrieval_mode and bool(
                getattr(settings, "RAG_DISABLE_INTENT_ROUTER_WHEN_PURE", True)
            ):
                skip_intent_router = True
            if pure_retrieval_mode and bool(
                getattr(settings, "RAG_DISABLE_HYDE_WHEN_PURE", True)
            ):
                skip_hyde = True
            if pure_retrieval_mode and bool(
                getattr(settings, "RAG_DISABLE_SUMMARIZE_WHEN_PURE", True)
            ):
                skip_summarize = True

            disable_query_rewrite_when_pure = bool(
                pure_retrieval_mode
                and getattr(settings, "RAG_DISABLE_QUERY_REWRITE_WHEN_PURE", True)
            )
            disable_low_score_fallback_rewrite_when_pure = bool(
                pure_retrieval_mode
                and getattr(
                    settings, "RAG_DISABLE_LOW_SCORE_FALLBACK_REWRITE_WHEN_PURE", True
                )
            )
            ...
            cache_verify_debug: Dict[str, Any] = {
                ...
                "semantic_cache_enabled": bool(
                    getattr(settings, "RAG_SEMANTIC_CACHE_ENABLED", True)
                ),
                "cache_verify_enabled": False,
                "cache_verify_passed": None,
                "cache_verify_reason": "not_checked",
                "cache_verify_min_term_overlap": int(
                    getattr(settings, "RAG_CACHE_VERIFY_MIN_TERM_OVERLAP", 1)
                ),
                "cache_verify_term_overlap": 0,
                "cache_verify_min_rerank_score": float(
                    getattr(settings, "RAG_CACHE_VERIFY_MIN_RERANK_SCORE", 0.25)
                ),
                "cache_verify_rerank_score": None,
            ...
            query_hash = hashlib.md5(
                f"{cache_namespace}|{raw_query_hash}".encode()
            ).hexdigest()
            ...
            query_vector = await asyncio.to_thread(
                self.vector_store.embedding_service.get_embedding, query
            )
            ...
            if self.redis_client:
                ...
                if semantic_enabled:
                    try:
                        ...
                        logger.warning(
                            "semantic_cache_lookup_timeout_degrade_to_retrieval",
                            timeout_s=1.5,
                        )
                        semantic_hit_hash = None
                    except Exception as semantic_err:
                        logger.warning(
                            "semantic_cache_lookup_failed_degrade_to_retrieval",
                            error=str(semantic_err),
                        )
                        semantic_hit_hash = None
                else:
                    semantic_hit_hash = None
                    cache_verify_debug["cache_verify_reason"] = (
                ...
                if semantic_hit_hash and semantic_hit_hash != query_hash:
                    ...
                    try:
                        reject_memory_hit = bool(
                            await asyncio.wait_for(
                                self.redis_client.exists(reject_memory_key), timeout=0.5
                            )
                        )
                    except Exception as reject_memory_err:
                        logger.warning(
                            "rag_reject_memory_check_failed",
                            error=str(reject_memory_err),
                        )
                    ...
                    if reject_memory_hit:
                        ...
                        cache_verify_debug["cache_verify_reason"] = (
                            "semantic_reject_memory_blocked"
                        )
                        logger.info(
                ...
                read_cache_key = (
                    f"rag_cache:{cache_namespace}:{effective_hash}:{cache_profile_hash}"
                )

                try:
                    cached_data = await asyncio.wait_for(
                        self.redis_client.get(read_cache_key), timeout=1.0
                    )
                except Exception as cache_read_err:
                    logger.warning(
                        "rag_cache_read_failed_degrade_to_retrieval",
                        error=str(cache_read_err),
                    )
                    cached_data = None

                if (
                    not cached_data
                    and semantic_hit_hash
                    and semantic_hit_hash != query_hash
                ):
                    exact_cache_key = (
                        f"rag_cache:{cache_namespace}:{query_hash}:{cache_profile_hash}"
                    )
                    try:
                    ...
                    if exact_cached_data:
                        ...
                        cache_verify_debug["cache_verify_reason"] = (
                            "semantic_key_miss_exact_fallback_hit"
                        )
                        logger.info(
                            "rag_exact_cache_fallback_hit",
                            query=query[:30],
                            stage="semantic_key_miss",
                        )
                ...
                if cached_data:
                    ...
                    try:
                        ...
                        logger.warning(
                            "rag_cache_payload_decode_failed_degrade_to_retrieval",
                            error=str(cache_decode_err),
                        )
                        results = []
                        cached_data = None
                    ...
                    if cached_data and semantic_hit_hash:
                        if getattr(settings, "RAG_CACHE_VERIFY_ON_HIT", True):
                            ...
                            (
                                verified,
                                verify_fields,
                            ) = await self._verify_semantic_cache_hit(query, results)
                            ...
                            if verified:
                                logger.info(
                                    ...
                                    query=query[:30],
                                    reason=cache_verify_debug.get(
                                        "cache_verify_reason"
                                    ),
                                    term_overlap=cache_verify_debug.get(
                                        "cache_verify_term_overlap"
                                    ),
                                    rerank_score=cache_verify_debug.get(
                                        "cache_verify_rerank_score"
                                    ),
                                )
                            else:
                                logger.warning(
                                    ...
                                    min_term_overlap=cache_verify_debug.get(
                                        "cache_verify_min_term_overlap"
                                    ),
                                    rerank_score=cache_verify_debug.get(
                                        "cache_verify_rerank_score"
                                    ),
                                    min_rerank_score=cache_verify_debug.get(
                                        "cache_verify_min_rerank_score"
                                    ),
                                )
                                reject_memory_key = self._build_reject_memory_key(
                                    cache_namespace=cache_namespace,
                                ...
                                cache_verify_debug["cache_reject_memory_key"] = (
                                    reject_memory_key
                                )
                                try:
                                    reject_reason = str(
                                        cache_verify_debug.get(
                                            "cache_verify_reason", "semantic_rejected"
                                        )
                                    )
                                    reject_ttl = max(
                                        60,
                                    ...
                                    await asyncio.wait_for(
                                        self.redis_client.setex(
                                            reject_memory_key, reject_ttl, reject_reason
                                        ),
                                        timeout=0.8,
                                    )
                                except Exception as reject_write_err:
                                    logger.warning(
                                        "rag_reject_memory_write_failed",
                                        error=str(reject_write_err),
                                    )
                                cached_data = None
                                ...
                                if semantic_hit_hash != query_hash:
                                    ...
                                    if exact_cached_data:
                                        try:
                                            exact_results = json.loads(
                                                self.semantic_cache.decompress(
                                                    exact_cached_data
                                                )
                                            )
                                            cached_data = exact_cached_data
                                            ...
                                            cache_verify_debug[
                                                "cache_verify_reason"
                                            ] = "semantic_rejected_exact_fallback_hit"
                                            logger.info(
                                                "rag_exact_cache_fallback_hit",
                                                query=query[:30],
                                            )
                        ...
                        cache_verify_debug["cache_verify_reason"] = (
                            "exact_cache_hit_skip_verify"
                        )

                    if cached_data:
                        pure_penalty_debug = self._default_pure_dept_penalty_debug()
                        if pure_retrieval_mode and results:
                            results, pure_penalty_debug = (
                                self._apply_pure_department_mismatch_penalty(
                                    query,
                                    results,
                            ...
                            pure_penalty_debug["pure_dept_penalty_reason"] = (
                                "pure_mode_disabled"
                            )
            ...
            # L1: 安全拦截 (Guardrail)
            if self.ddinter_checker and not self.ddinter_checker.check_query_safety(
                query
            ):
                logger.warning("query_blocked_by_guardrail")
                res = [
                    {
                        "content": "您的查询可能涉及高风险用药建议，已被安全护栏拦截。",
                        "score": 1.0,
                        "source": "guardrail",
                    }
                ]
            ...
            if not skip_intent_router:
                ...
                if category == "GREETING":
                    ...
                    res = [
                        {
                            "content": "您好！我是您的智能医疗助手。请问有什么可以帮您？",
                            "score": 1.0,
                            "source": "greeting",
                        }
                    ]
            ...
            hyde_doc = None
            if (
                not skip_hyde and len(query) < 20
            ):  # Only apply HyDE for short/ambiguous queries
                try:
            ...
            if hyde_doc:
                try:
                    hyde_vector = await asyncio.to_thread(
                        self.vector_store.embedding_service.get_embedding, hyde_doc
                    )
                    # Weighted Fusion: 70% Original + 30% HyDE
                    if isinstance(query_vector, list) and isinstance(hyde_vector, list):
                        final_vector = [
                            0.7 * q + 0.3 * h for q, h in zip(query_vector, hyde_vector)
                        ]
            ...
            recall_window = max(
                int(getattr(settings, "RAG_RECALL_WINDOW", 100)), int(top_k)
            )
            ...
            bm25_task = asyncio.to_thread(
                self.bm25_indexer.search, query, top_k=recall_window
            )
            ...
            RAG_RETRIEVAL_LATENCY.labels(stage="milvus_parallel").observe(
                milvus_duration
            )
            ...
            rerank_candidate_k = max(
                int(top_k), int(getattr(settings, "RAG_RERANK_CANDIDATE_K", 12))
            )
            ...
            fused_pool = self._rrf_fusion(
                v_results, b_results, top_k=fusion_pool_k, target_dept=target_dept
            )
            pure_candidate_comp_debug = self._default_pure_candidate_comp_debug()
            if pure_retrieval_mode and fused_pool:
                results, pure_candidate_comp_debug = (
                    self._select_pure_rerank_candidates(
                        query=query,
                        ...
                        async def _fetch_content_map_by_ids(ids_list):
                            ...
                            try:
                                async with AsyncSessionLocal() as session:
                                    ...
                                    placeholders = ", ".join(
                                        [f":id{i}" for i in range(len(ids_list))]
                                    )
                                    query_variants = [
            ...
            if self.reranker and len(results) > 0 and use_rerank is not False:
                ...
                if len(results) == 1:
                    ...
                    results[0].get("score", 0) > 0.9
                    and (results[0].get("score", 0) - results[1].get("score", 0)) > 0.2
                ):
                    skip_rerank = True
                    logger.info(
                        "adaptive_rerank_skipped", top_score=results[0].get("score")
                    )
                ...
                if not skip_rerank:
                    ...
                    results = await asyncio.to_thread(
                        self.reranker.rerank, query, truncated_results
                    )
                    ...
                    # [Metrics] Latency Tracking
                    RAG_RETRIEVAL_LATENCY.labels(stage="reranker").observe(
                        rerank_duration
                    )
                else:
                ...
                missing_dept_ids = [
                    r["id"]
                    for r in results
                    if r.get("department") in [None, "Unknown", "None", "", "null"]
                ]
                if missing_dept_ids:
                    try:
                        async with AsyncSessionLocal() as session:
                            params = {
                                f"id{i}": int(id_val)
                                for i, id_val in enumerate(missing_dept_ids)
                            }
                            placeholders = ", ".join(
                                [f":id{i}" for i in range(len(missing_dept_ids))]
                            )
                            query_variants = [
                ...
                BASE_REL_THRESHOLD = (
                    0.15  # [Optimization] 进一步降低阈值以减少不必要的重试延迟
                )
                if rerank_threshold is not None:
                    try:
                        ...
                        logger.warning(
                            "invalid_rerank_threshold_fallback_default",
                            rerank_threshold=rerank_threshold,
                        )
                effective_rel_threshold = BASE_REL_THRESHOLD
                if pure_retrieval_mode:
                    pure_factor = float(
                        getattr(settings, "RAG_PURE_RERANK_THRESHOLD_FACTOR", 0.75)
                    )
                ...
                if not results or results[0]["score"] < effective_rel_threshold:
                    logger.warning(
                        "rerank_blocked_by_threshold_triggering_fallback",
                    ...
                    if (
                        not intent == "fallback_retry"
                        and not disable_low_score_fallback_rewrite_when_pure
                    ):
                        rewritten_query = await self._rewrite_query_async(query)
                        if rewritten_query and rewritten_query != query:
                            logger.info(
                                "rag_fallback_retry_start",
                        ...
                        logger.info(
                            "rag_fallback_retry_skipped_pure_mode", query=query[:60]
                        )
                ...
                for j in range(1, min(len(results), MAX_FINAL_K)):
                    if (
                        results[j]["score"] >= effective_rel_threshold
                        and (results[j - 1]["score"] - results[j]["score"])
                        < GAP_THRESHOLD
                    ):
                        final_count += 1
                    else:
                ...
                logger.info(
                    "dynamic_k_selected",
                    selected_k=final_count,
                    top_score=results[0]["score"],
                )

                print(
                    f"[DEBUG] RAG Found {len(results)} docs. Top Score: {results[0]['score']:.4f}"
                )
            ...
            if self.ddinter_checker:
                ...
                warnings = await self.ddinter_checker.scan_query_for_warnings_async(
                    query
                )
                if warnings:
                    warning_msg = "\n".join(warnings)
            ...
            if final_results and not skip_summarize:
                ...
                for i, r in enumerate(final_results):
                    # 仅对来自检索源且未被摘要的正文进行处理
                    if (
                        r.get("source") in ["reranked", "vector", "bm25"]
                        and len(r.get("content", "")) > 200
                    ):
                ...
                if texts_to_summarize:
                    ...
                    try:
                        summaries = await self.summarize_batch(
                            texts_to_summarize, query, low_confidence=low_confidence
                        )
                        for idx, summary in zip(to_summarize_indices, summaries):
                            final_results[idx]["content"] = summary
                            final_results[idx]["source"] = (
                                f"{final_results[idx].get('source')}_summarized"
                            )
                            ...
                            if (
                                low_confidence
                                and "以上信息仅供参考"
                                not in final_results[idx]["content"]
                            ):
                    ...
                    if (
                        final_results
                        and "以上信息仅供参考" not in final_results[0]["content"]
                    ):
            ...
            cache_write_allowed, cache_write_debug = self._evaluate_cache_write_gate(
                final_results
            )
            ...
            if (
                ...
                try:
                    ...
                    compressed = self.semantic_cache.compress(
                        json.dumps(final_results, ensure_ascii=False)
                    )
                    cache_ttl = max(
                        60, int(getattr(settings, "RAG_CACHE_TTL_SECONDS", 86400))
                    )
                    await asyncio.wait_for(
                        self.redis_client.setex(cache_key, cache_ttl, compressed),
                        timeout=1.0,
                    )
                    semantic_enabled = bool(
                        getattr(settings, "RAG_SEMANTIC_CACHE_ENABLED", True)
                    ...
                    if semantic_enabled:
                        ...
                        await asyncio.wait_for(
                            asyncio.to_thread(
                                self.semantic_cache.update_cache,
                                query,
                                query_hash,
            ...
            RAG_RETRIEVAL_LATENCY.labels(stage="total").observe(
            ...
            logger.error(
                "rag_pipeline_critical_error",
                error=str(outer_e),
                stack=traceback.format_exc(),
            )
            return ([], {}) if return_debug else []

    async def _extract_department_filter_async(
        self, query: str
    ...
    async def _rewrite_query_async(self, query: str) -> str:
        ...
        try:
            ...
            results = await local_slm.generate_batch_async(
                [prompt], system_prompt="You are a medical query rewriter."
            )
    ...
    def _extract_department_filter(
        self, query: str
    ) -> tuple[Optional[List[int]], Optional[str]]:
    ...
    def _heavy_lifting_search(
        ...
        v_results = self.vector_store.search(
            query, top_k=top_k * 2, filter_ids=dept_ids
        )
        b_results = self.bm25_indexer.search(query, top_k=top_k * 2)
        return self._rrf_fusion(
            v_results, b_results, top_k=top_k, target_dept=target_dept
        )
    ...
    def _legacy_search_sync_wrapper(
        self, query: str, top_k: int = 3, intent: str = None
    ) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if loop.is_running():
                    return loop.run_until_complete(
                        self.search_rag30(query, top_k, intent)
                    )
    ...
    def search_sync(
        ...
        pipeline_enabled = bool(
            getattr(settings, "UPGRADE3_RETRIEVER_PIPELINE_ENABLED", True)
        )
        logger.info(
        ...
        if pipeline_enabled:
            return retrieval_pipeline.search_sync(
                self, query=query, top_k=top_k, intent=intent
            )
...
Answer:"""
    ...
    async def summarize_batch(
        self, texts: List[str], query: str, low_confidence: bool = False
    ) -> List[str]:
        """
        ...
        """
        print(
            f"[DEBUG] Node MedicalRetriever.summarize_batch Start (Batch Size: {len(texts)}, Low Confidence: {low_confidence})"
        )
        if not texts:
            return []
